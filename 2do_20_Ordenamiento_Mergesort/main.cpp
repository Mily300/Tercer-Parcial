...
void ordenarEnteros(Logger& log) {
    ...
    mergeSort(punteros, 0, n - 1);
    cout << "\n[RESULTADO] Enteros ordenados: "; 
    imprimirArreglo(punteros, n);
    ostringstream ss;
    for(int i = 0; i < n; i++) ss << *punteros[i] << (i < n-1 ? " | " : "");
    log.registrar("mergesort_enteros", "Ordenados " + to_string(n) + " enteros", ss.str());
...
void ordenarCaracteres(Logger& log) {
    ...
    ostringstream ss;
    for(int i = 0; i < n; i++) ss << *punteros[i] << (i < n-1 ? " | " : "");
    log.registrar("mergesort_caracteres", "Ordenados " + to_string(n) + " caracteres", ss.str());
    delete[] punteros; delete[] datos;
}
...
void ordenarPersonasPOO(Logger& log) {
    ...
    log.registrar("mergesort_personas", "Ordenadas " + to_string(n) + " personas por edad", ss.str());
    delete[] punteros; delete[] datos;
}

int menu() {
    int opcion;
    cout << "\nMenu Merge Sort Indirecto (POO)\n";
...
int main() {
    int opcion;
    Logger log("2do_20-Ordenamiento MergeSort POO");
